...
const AddAdminForm = () => {
  ...
  const getGuruById = async () => {
    ...
    try {
      const response = await axios.get(assets.API_LINK + "/admin/" + id_admin, {
  ...
  const cekPassword = async () => {
    try {
      const response = await axios.post(
        assets.API_LINK + "/admin/cekPassword/" + id_admin,
        { password },
        { headers: { Authorization: `Bearer ${token}` } }
      );
    ...
    try {
      ...
      if (validPassword) {
        const response = await axios.put(
          assets.API_LINK + "/admin/" + id_admin,
          {
            nama,
            email,
            password,
        ...
        if (response.data.message == "Admin updated successfully") {
          router.push("/admin/informasi-admin");
        } else {
          setMsg(response.data.message);
        }
      }
