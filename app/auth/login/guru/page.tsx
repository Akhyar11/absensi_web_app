...
const Login = () => {
  ...
  const handleLogin = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (
        loginResponse.data.message !== "Guru not found" ||
        loginResponse.data.message == "Invalid password"
      ) {
        Cookies.set("token", loginResponse.data.message, {
          expires: 1000 * 60 * 60,
        });

        router.push("/guru/dashboard");
      } else {
        setMsg(loginResponse.data.message || "Login successful");
      }
      setIsLoading(false);
    } catch (error) {
      setIsLoading(false);
